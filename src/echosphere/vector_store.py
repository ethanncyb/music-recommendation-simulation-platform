...
- ``get_collection(persist_dir=...)`` — idempotent, returns the Chroma
  collection, auto-seeding from ``data/songs.csv`` on first access.
- ``ingest_catalog(csv_path, client=None)`` — (re)builds the collection from a
  CSV file. Safe to call repeatedly.
...
def get_collection(
    ...
    """Return the EchoSphere collection, seeding it from CSV if empty."""
    ...
    if auto_ingest and collection.count() == 0 and Path(csv_path).exists():
        ingest_catalog(csv_path=csv_path, client=client, collection=collection)
...
def _coerce_row(row: Dict[str, str]) -> Dict[str, Any]:
    """Convert CSV string fields into floats / ints where appropriate."""
    out: Dict[str, Any] = {}
...
def ingest_catalog(
    ...
    """Idempotently (re)build the EchoSphere collection from ``csv_path``.
...
def _main() -> None:
    ...
    parser.add_argument("--csv", default=DEFAULT_CSV_PATH, help="Catalog CSV path")
    parser.add_argument(
        "--persist-dir",
    ...
    summary = ingest_catalog(csv_path=args.csv, persist_dir=args.persist_dir)
